...
exports.OVERRIDE_LIMIT_PER_DAY = exports.MIN_OVERRIDE_REASON_LEN = exports.OVERRIDE_CREW = exports.CREW_CREDENTIAL_REGISTRY = exports.WORFGATE_OFFICER = void 0;
exports.getCredentialAuditLog = getCredentialAuditLog;
exports.registerCredentialProvider = registerCredentialProvider;
exports.listCredentialProviders = listCredentialProviders;
exports.worfGateHasCredential = worfGateHasCredential;
exports.resolveWorfGateCredential = resolveWorfGateCredential;
exports.resolveWorfGateCredentialAsync = resolveWorfGateCredentialAsync;
exports.redactCredential = redactCredential;
exports.credentialStatus = credentialStatus;
exports.getOverrideAuditLog = getOverrideAuditLog;
exports.resolveWorfGateOverride = resolveWorfGateOverride;
exports.summarizeOverridesForLounge = summarizeOverridesForLounge;
...
const credentialAuditLog = [];
const AUDIT_MAX = 500;
function audit(entry) {
    credentialAuditLog.push(entry);
    if (credentialAuditLog.length > AUDIT_MAX)
        credentialAuditLog.splice(0, credentialAuditLog.length - AUDIT_MAX);
}
/** The credential-access audit trail (no values, ever). */
function getCredentialAuditLog() {
    return [...credentialAuditLog];
}
const PROVIDERS = [];
/** Register a secret provider (sorted by priority; lower first). */
function registerCredentialProvider(p) {
...
function listCredentialProviders() {
...
registerCredentialProvider({
    ...
    priority: 100,
    isActive: () => true,
    get: async (name) => process.env[name] || undefined,
});
/** Shared authorization gate (no audit, no resolution). */
function checkAuthorization(name, operation, crewId) {
    const spec = exports.CREW_CREDENTIAL_REGISTRY[name];
    if (!spec)
        return { authorized: false, reason: `'${name}' is not in the WorfGate credential registry` };
    if (!spec.operations.includes(operation))
        return { authorized: false, reason: `'${name}' is not permitted for operation '${operation}'` };
    if (!AUTHORIZED_CREW.has(crewId))
        return { authorized: false, reason: `crew member '${crewId}' is not authorized to broker credentials` };
    return { authorized: true, reason: 'authorized' };
}
/** Presence-only check across the env (sync, no value). */
function worfGateHasCredential(name) {
 ...
 */
function resolveWorfGateCredential(name, ctx) {
    const crewId = (ctx.crewId || exports.WORFGATE_OFFICER).toLowerCase();
    ...
    if (!auth.authorized)
        return { name, authorized: false, available, reason: auth.reason };
    if (!available)
        return { name, authorized: true, available: false, reason: `'${name}' not present in env — add to ~/.zshrc / ~/.alexai-secrets, or configure a provider (Vault/Secrets Manager)` };
 ...
 */
async function resolveWorfGateCredentialAsync(name, ctx) {
    const crewId = (ctx.crewId || exports.WORFGATE_OFFICER).toLowerCase();
    const auth = checkAuthorization(name, ctx.operation, crewId);
    if (!auth.authorized) {
...
function redactCredential(r) {
    const { value, ...rest } = r;
    return { ...rest, hasValue: Boolean(value) };
}
/** Presence map for a set of credentials (no values) — for status/inventory views. */
function credentialStatus(names) {
    const keys = names && names.length ? names : Object.keys(exports.CREW_CREDENTIAL_REGISTRY);
    return keys
        .map(k => exports.CREW_CREDENTIAL_REGISTRY[k])
        .filter(Boolean)
...
exports.OVERRIDE_CREW = new Set(['obrien', 'worf', 'picard']);
exports.MIN_OVERRIDE_REASON_LEN = 20;
exports.OVERRIDE_LIMIT_PER_DAY = 3;
const OVERRIDE_WINDOW_MS = 24 * 60 * 60 * 1000;
const overrideAuditLog = [];
const OVERRIDE_AUDIT_MAX = 500;
function recordOverride(e) {
 ...
 */
function resolveWorfGateOverride(name, ctx) {
    const crewId = (ctx.crewId || '').toLowerCase();
    const reason = (ctx.reason || '').trim();
    const ts = new Date().toISOString();
    const deny = (denialReason, available = false) => {
    ...
    if (!exports.OVERRIDE_CREW.has(crewId))
        return deny(`'${crewId}' is not permitted to break-glass override (O'Brien/Worf/Picard only)`);
    if (!exports.CREW_CREDENTIAL_REGISTRY[name])
        return deny(`'${name}' is not a registered credential — override never invents secrets`);
    if (reason.length < exports.MIN_OVERRIDE_REASON_LEN)
        return deny(`a justification of ≥ ${exports.MIN_OVERRIDE_REASON_LEN} chars is required`);
    ...
    if (recent >= exports.OVERRIDE_LIMIT_PER_DAY)
        return deny(`rate limit reached (${exports.OVERRIDE_LIMIT_PER_DAY}/24h) — escalate to the crew in the Observation Lounge`);
    const value = process.env[name];
    const available = Boolean(value);
    ...
    if (!available)
        return { name, authorized: true, available: false, reason: `override GRANTED but '${name}' absent in env`, override: true, monitored: true };
...
function summarizeOverridesForLounge(sinceHours = 24) {
    ...
    for (const [crew, n] of Object.entries(byCrew)) {
        if (n >= exports.OVERRIDE_LIMIT_PER_DAY)
            anomalies.push(`${crew} at/over the ${exports.OVERRIDE_LIMIT_PER_DAY}/24h override limit (${n})`);
    }
    for (const [k, n] of Object.entries(byCredentialOp)) {
        if (n >= 2)
            anomalies.push(`repeated override ${k} ×${n} — review the underlying allowlist gap`);
    }
    if (denied > 0)
        anomalies.push(`${denied} override(s) DENIED — inspect for probing/misconfig`);
    return { windowHours: sinceHours, total: recent.length, granted, denied, byCrew, byCredentialOp, recent, anomalies };
}
//# sourceMappingURL=worfgate-credentials.js.map