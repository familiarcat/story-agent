...
export type AsyncState = 'queued' | 'running' | 'done' | 'failed' | 'timeout';
...
/** Derived current state of one unit of work. */
...
export function asyncLogPath(dir: string): string { return `${dir}/${LOG}`; }
...
export const DEFAULT_STALE_TERMINAL_MS = 600_000;

let seq = 0;

function append(dir: string, ev: AsyncEvent): void {
  try { appendFileSync(asyncLogPath(dir), JSON.stringify(ev) + '\n'); } catch { /* best-effort */ }
}

/** Start tracking a unit of async work. Returns the generated id for heartbeat/end. Never throws. */
export function beginAsync(
  dir: string,
  opts: { kind: string; label: string; clientId?: string; timeoutMs?: number },
  now: number,
): string {
...
export function heartbeatAsync(dir: string, id: string, patch: { progress?: number }, now: number): void {
  append(dir, { ev: 'beat', id, ts: now, ...(patch.progress != null ? { progress: patch.progress } : {}) });
}

/** Mark a unit terminal. Never throws. */
export function endAsync(dir: string, id: string, state: 'done' | 'failed' | 'timeout', now: number): void {
  append(dir, { ev: 'end', id, state, ts: now });
}

/** Tolerant read of the raw event log (chronological). Returns [] if absent/unreadable. */
export function readAsyncEvents(dir: string): AsyncEvent[] {
  let raw = '';
  try { raw = readFileSync(asyncLogPath(dir), 'utf8'); } catch { return []; }
  const out: AsyncEvent[] = [];
  for (const line of raw.split('\n')) {
    const t = line.trim();
    if (!t) continue;
    try { out.push(JSON.parse(t) as AsyncEvent); } catch { /* skip malformed */ }
  }
  return out;
}
 ...
 */
export function foldAsyncEvents(
  events: AsyncEvent[],
  now: number,
  opts?: { staleTerminalMs?: number },
): AsyncEntry[] {
  const staleTerminalMs = opts?.staleTerminalMs ?? DEFAULT_STALE_TERMINAL_MS;
  const map = new Map<string, AsyncEntry>();
  for (const e of events) {
    if (e.ev === 'begin') {
      ...
      if (!cur) continue; // beat/end before begin — ignore
      cur.updatedAt = e.ts;
      if (e.progress != null) cur.progress = e.progress;
      if (e.ev === 'end' && e.state) { cur.state = e.state; cur.endedAt = e.ts; }
    }
  }

  const entries: AsyncEntry[] = [];
  for (const cur of map.values()) {
    ...
    if (terminal && now - (cur.endedAt ?? cur.updatedAt) > staleTerminalMs) continue;
    entries.push(cur);
  }

  const rank: Record<AsyncState, number> = { running: 0, queued: 1, timeout: 2, failed: 3, done: 4 };
  entries.sort((a, b) => (rank[a.state] - rank[b.state]) || (b.updatedAt - a.updatedAt));
  return entries;
}

/** Read + fold in one call. */
export function readAsyncState(dir: string, now: number, opts?: { staleTerminalMs?: number }): AsyncEntry[] {
  return foldAsyncEvents(readAsyncEvents(dir), now, opts);
}

const ICON: Record<AsyncState, string> = { running: '▶', queued: '▷', done: '✓', failed: '✗', timeout: '⏱' };
 ...
 */
export function formatAsyncSnapshot(
  entries: AsyncEntry[],
  now: number,
  opts?: { max?: number; header?: boolean },
): string {
 ...
 */
export function pruneAsyncLog(dir: string, now: number, opts?: { retentionMs?: number }): void {
  ...
  try {
    ...
    for (const e of events) {
      ...
      if (e.ev === 'end') ended.add(e.id);
    }
    const keepIds = new Set<string>();
    for (const [id, ts] of lastTs) {
      if (!ended.has(id) || now - ts <= retentionMs) keepIds.add(id);
    }
    if (keepIds.size === lastTs.size) return; // nothing to prune
