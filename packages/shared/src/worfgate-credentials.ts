...
export const CREW_CREDENTIAL_REGISTRY: Record<string, CredentialSpec> = {
  ...
  STORY_AGENT_S3_BUCKET: { name: 'STORY_AGENT_S3_BUCKET', description: 'Application S3 bucket root for client/project/sprint content and static crew files RAG cannot hold. Deterministically named story-agent-{account}-{region} and provisioned by scripts/setup-app-bucket.sh — resolve this rather than hardcoding a bucket name anywhere.', operations: ['aws:deploy', 'aws:secrets'], required: false },
  STORY_AGENT_OAUTH_SIGNING_KEY: { name: 'STORY_AGENT_OAUTH_SIGNING_KEY', description: 'Symmetric HMAC secret (HS256) used to sign and verify the OAuth 2.1 access/refresh tokens the deployed MCP server issues to external clients (e.g. Claude.ai). Same service both issues and verifies these tokens, so a symmetric key is sufficient — revisit if a third service ever needs to verify them independently. Generate with e.g. `openssl rand -base64 48`; rotating it invalidates every outstanding token.', operations: ['aws:secrets'], required: false },
  STORY_AGENT_OAUTH_OWNER_PASSPHRASE: { name: 'STORY_AGENT_OAUTH_OWNER_PASSPHRASE', description: 'Shared passphrase gating the OAuth 2.1 /authorize consent step (single-owner trust model — this is not a full identity provider). Required for the "Authorize" endpoint to ever issue a code; unset means every authorization attempt fails closed.', operations: ['aws:secrets'], required: false },
