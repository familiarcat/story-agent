...
export const CREW_BASELINE_MEMORIES = {
  picard: {
...
export function getCrewMemoryStory(crewId: string) {
  const crew = CREW_BASELINE_MEMORIES[crewId as keyof typeof CREW_BASELINE_MEMORIES];
  if (!crew) return null;
...
export function getAllCrewMemories() {
  return Object.entries(CREW_BASELINE_MEMORIES).map(([crewId, crew]) => ({
    crewId,
    role: crew.role,
