...
/**
 * Batch API Routes for Stories
 */

import { NextRequest, NextResponse } from 'next/server';
import { createStory, updateStory, deleteStory } from '@/lib/pm-db';
...
import type { Story } from '@story-agent/shared/pm-contracts';

/**
 * POST /api/stories/batch
 * Create, update, or delete multiple stories in a single request
 */
export async function POST(request: NextRequest): Promise<NextResponse> {
  try {
    ...
    for (let i = 0; i < operations.length; i++) {
      ...
      try {
        if (op.type === 'create') {
          const result = await createStory(tenantId, userId, {
            tenant_id: tenantId,
            ...op.data,
          });
          results.push({ index: i, status: 201, data: result });
        } else if (op.type === 'update') {
          const result = await updateStory(tenantId, userId, op.id, op.data);
          results.push({ index: i, status: 200, data: result });
        } else if (op.type === 'delete') {
          await deleteStory(tenantId, userId, op.id);
