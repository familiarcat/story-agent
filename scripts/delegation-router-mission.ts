...
const p=`docs/observation-lounge/delegation-router-${stamp}.md`; writeFileSync(p,md); console.log('TRANSCRIPT '+p);
const obs=await storeObservationMemory({storyId:'delegation-router',source:'mcp',transcript:{rounds:[{title:'delegation router',entries:r.contributions.map(c=>({speakerId:c.crewId,position:'support',statement:c.text,evidence:[c.model]}))}],consensusSummary:r.missionPlan,unresolvedRisks:['hook sees every prompt (Worf)','quality regression on cheap routing (Yar)'],finalDecision:'approved',actionItems:['Ship the complexity/cost scorer + first delegation artifact']},tags:['cost','delegation','router','algorithm','claude-code','openrouter']});
console.log('OBS '+obs.id+' emb='+embeddingSource());
const m=await storeCrewPersonalMemory({crew_id:'data',memory_type:'decision_note',title:'Delegation Router — reusable complexity/cost scorer + first automatic-delegation artifact',content:r.missionPlan,tags:['cost','delegation','router','algorithm'],relates_to_crew:['quark','geordi','worf','yar','riker','picard']});
console.log('MEM '+m);
