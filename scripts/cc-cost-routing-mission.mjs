  ...
  rounds: [
    ...
    },
    {
      ...
      entries: r.contributions.map((c) => ({
        ...
        position: 'support',
        statement: c.text,
        evidence: [`model:${c.model}`, `cost:$${c.costUSD.toFixed(6)}`],
      })),
    },
    {
      ...
      entries: [
        {
          ...
          evidence: [`topModel:${r.topModel}`, `totalCost:$${r.efficiency.totalCostUSD.toFixed(6)}`],
        },
      ],
    },
  ],
  consensusSummary: r.missionPlan,
  unresolvedRisks: ['proxy feature parity (tool-use/streaming/caching)'],
  finalDecision: 'approved',
  actionItems: ['Decide delegation-automation vs Anthropic-compatible Quark proxy'],
};
const obs=await storeObservationMemory({
  storyId:'cc-cost-routing',
  source:'mcp',
  transcript: debate,
  missionReference: 'cc-cost-routing',
  tags:['cost','claude-code','openrouter','quark','routing','proxy','mission-parameters','resolution'],
});
console.log('OBS '+obs.id+' emb='+embeddingSource());
const m=await storeCrewPersonalMemory({crew_id:'quark',memory_type:'decision_note',title:'How to actually cut Claude Code cost via the OpenRouter/Quark crew (delegation vs proxy)',content:`Mission brief: ${BRIEF}\n\nResolution: ${r.missionPlan}`,tags:['cost','claude-code','openrouter','proxy','mission-parameters','resolution'],relates_to_crew:['picard','geordi','riker','worf','troi','data']});
console.log('MEM '+m);
