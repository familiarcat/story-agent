 ...
 */
import { selectVisionModel } from '@story-agent/shared';
...
const blended = (m) => m.costIn + m.costOut; // simple cost proxy for ranking
/** QUARK: cheapest model in the pool meeting (>=) the required capability tier. */
export function quarkSelectModel(capabilityTier) {
...
export function quarkCheapestAnthropic() {
...
export function quarkSelectVisionModel(complexity, opts) {
...
export function crewBaseTier(crewId) {
 ...
 */
export function escalatedTierForBusinessTier(businessTier) {
    if (businessTier === 'enterprise')
        return 4;
    if (businessTier === 'commercial')
        return 3;
    return 0;
}
/** Effective capability tier = MAX(crew base, requester-tier escalation). Enterprise → ≥4. */
export function effectiveCapabilityTier(crewId, requesterTier) {
    return Math.max(crewBaseTier(crewId), escalatedTierForBusinessTier(requesterTier));
}
/** QUARK selection escalated by the requester's tier — enterprise prompts get the leader/frontier model. */
export function quarkSelectModelForRequester(crewId, requesterTier) {
 ...
 */
export function assembleAndOptimize(issue, maxTier = 4) {
    ...
    if (isFallback)
        selected.push(...CREW.filter(c => ['data', 'riker'].includes(c.crewId)));
    const providerMix = {};
    const team = selected.map(c => {
        ...
        const capabilityTier = Math.min(maxTier, c.baseTier + (complex ? 1 : 0));
