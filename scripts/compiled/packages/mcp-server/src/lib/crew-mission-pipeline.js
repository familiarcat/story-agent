 ...
 */
import { assembleAndOptimize, quarkSelectModel, MODEL_POOL } from './crew-team-assembly.js';
...
export async function runMissionPipeline(nlInput, clientId, complexity) {
    if (!OR_KEY)
        throw new Error('CREW_LLM_APPROVED_KEY not set');
    const ledger = [];
    ...
    const asyncId = beginAsync(asyncDir, { kind: 'mission', label: 'run_crew_mission_pipeline', timeoutMs: 180000, ...(clientId ? { clientId } : {}) }, Date.now());
    try {
        ...
        const contributions = await Promise.all(plan.team.map(async (m) => {
            const r = await call(m.model, `You are ${m.crewId} (${m.domain}) of the Story Agent crew, in the Observation Lounge. Contribute YOUR domain's part toward the goals: a concrete position + one concern/resolution. 2-3 sentences.`, `GOALS:\n${goals}`, 160);
            ledger.push(r);
...
Flag any disagreement between the three approaches (e.g., "teams diverged on whether to migrate vs patch"). If no variance, say "Consensus across all three approaches."`;
        const alternativesResp = await call(TOP_MODEL, alternativesPrompt, `GOALS:\n${goals}\n\nCREW CONTRIBUTIONS:\n${contributions.map(c => `${c.crewId}: ${c.text}`).join('\n')}`, 900);
