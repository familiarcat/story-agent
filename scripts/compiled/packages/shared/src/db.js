...
export async function flushObservationMemoryQueue(batchSize = REDIS_SYNC_BATCH_SIZE) {
    const client = await redis();
    if (!client) {
    ...
    for (const p of payloads)
        byHash.set(p.transcript_hash, p);
...
export async function getObservationMemorySyncDiagnostics() {
    const client = await redis();
...
export async function getSupabaseConnectivityDiagnostics() {
    const { mode, candidates } = getSupabaseCandidates();
    if (candidates.length === 0) {
        return {
            mode,
...
export function stopObservationMemorySyncWorker() {
    if (_memorySyncTimer) {
...
export async function setEphemeral(key, value, ttlSeconds = EPHEMERAL_DEFAULT_TTL_SECONDS) {
    const client = await redis();
    if (!client)
        return false;
...
export async function upsertStory(clientId, story) {
...
export async function getStory(storyId, clientId) {
...
// ── PR Comments ──────────────────────────────────────────────────────────────
export async function upsertPRComments(comments) {
...
export async function getCommentsForStory(storyId) {
...
// ── Revision Cycles ──────────────────────────────────────────────────────────
export async function createRevisionCycle(cycle) {
...
export async function getRevisionCycles(storyId) {
...
export async function recordObservationMemoryOutcome(input) {
    const rows = throwOnError(await (await db())
        ...
        .limit(1));
    const row = Array.isArray(rows) ? rows[0] : rows;
    return row ? mapObservationMemory(row) : null;
}
export async function getRecentObservationMemories(limit = 8, storyId, clientId) {
 ...
 */
export async function getCrewBaselineMemory(crewId) {
 ...
 */
export async function getAllCrewBaselineMemories() {
    const client = await getClient();
    if (!client)
        return new Map();
    try {
...
export function formatMemoryForCrewDisplay(memory) {
    const lines = [];
 ...
 */
export async function searchDocumentation(query, category, limit = 10) {
...
export async function storeDocumentationChunk(rec) {
    ...
    if (error)
        throw new Error('storeDocumentationChunk: ' + error.message);
}
/**
 * Get documentation by category
 * Returns all documents in a specific category for browsing
 */
export async function getDocumentationByCategory(category) {
    const client = await getSupabaseClient();
 ...
 */
export async function searchDocumentationByEmbedding(embeddingVector, category, limit = 10, similarityThreshold = 0.7) {
    const client = await getSupabaseClient();
 ...
 */
export async function refreshDocumentationIndex() {
    const client = await getSupabaseClient();
 ...
 */
export async function getCrewPersonalMemories(crew_id, limit = 20, includePrivate = false) {
    const client = await getSupabaseClient();
 ...
 */
export async function searchCrewPersonalMemories(crew_id, query, limit = 10) {
    const client = await getSupabaseClient();
 ...
 */
export async function searchCrewPersonalMemoriesByEmbedding(crew_id, embeddingVector, limit = 10, similarityThreshold = 0.7) {
    const client = await getSupabaseClient();
 ...
 */
export async function getCrewMemoriesByProject(crew_id, project_id, limit = 20) {
    const client = await getSupabaseClient();
 ...
 */
export async function getCrewMemoryStats(crew_id) {
    const client = await getSupabaseClient();
