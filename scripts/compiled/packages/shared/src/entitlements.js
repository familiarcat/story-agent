...
let resolver = () => [];
export function setEntitlementResolver(r) {
    resolver = r;
}
const ORDER = ['tier', 'client', 'project', 'sprint'];
/** Does a single grant cover the requested resource path (top-down inheritance)? */
function grantCovers(grant, path, need) {
    if (grant.access === 'read' && need === 'write')
        return false;
    if (grant.scope === '*')
        return true;
 ...
 */
export async function checkHumanEntitlement(humanId, path, need = 'read') {
    if (!humanId?.trim())
        return { allowed: false, reason: 'No human identity provided.' };
