 ...
 */
const DEFAULT_NATIVE_RATE = { in: 3, out: 15 }; // frontier-class (Sonnet/Opus order of magnitude)
const DEFAULT_DELEGATE_RATE = { in: 0.25, out: 0.85 }; // tier-3 deepseek
 ...
 */
export function scoreDelegation(prompt, opts = {}) {
    ...
    if (trivial)
        complexity *= 0.4; // trivial language strongly pulls toward native
    ...
    const estCostDelegateUSD = (tokensInNative / 1e6) * delegateRate.in + (tokensOut / 1e6) * delegateRate.out + orchestrationOverheadUSD;
    const savingsUSD = Number((estCostNativeUSD - estCostDelegateUSD).toFixed(6));
    const tier = safetyGated ? 4 : 3; // delegation is frugal-capped at tier-3
    const mode = agentic ? 'agent' : reasoning ? 'deliberate' : null;
    let route;
    let reason;
    if (safetyGated) {
        route = 'native';
