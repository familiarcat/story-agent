...
import { buildClientPolicy, cacheClientPolicy, clearDynamicClientCache, lookupClientPolicy, resolveClientPolicy, } from './client-security-policy.js';
...
function rowToPolicy(row) {
    if (row.policy && typeof row.policy === 'object' && row.policy.clientId)
        return row.policy;
    return buildClientPolicy({
        clientId: row.id,
        clientName: row.name,
        tier: row.security_tier || 'enterprise',
 ...
 */
export async function onboardClient(spec, onboardedBy = 'crew') {
    ...
    if (policy.parentClientId)
        await ensureParentRow(db, policy.parentClientId, onboardedBy);
    await persistPolicyRow(db, policy, onboardedBy);
    cacheClientPolicy(policy);
    return policy;
}
/** Upsert a policy as a clients row. */
async function persistPolicyRow(db, policy, onboardedBy) {
...
export async function listClientsFromDb() {
    ...
    if (error)
        throw new Error(`listClientsFromDb: ${error.message}`);
    return (data ?? []);
}
