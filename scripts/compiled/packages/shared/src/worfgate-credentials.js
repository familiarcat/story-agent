...
export const CREW_CREDENTIAL_REGISTRY = {
...
export function registerCredentialProvider(p) {
...
export function worfGateHasCredential(name) {
 ...
 */
export function resolveWorfGateCredential(name, ctx) {
 ...
 */
export async function resolveWorfGateCredentialAsync(name, ctx) {
    const crewId = (ctx.crewId || WORFGATE_OFFICER).toLowerCase();
    const auth = checkAuthorization(name, ctx.operation, crewId);
    if (!auth.authorized) {
...
export function redactCredential(r) {
    const { value, ...rest } = r;
...
export function getOverrideAuditLog() {
 ...
 */
export function resolveWorfGateOverride(name, ctx) {
    ...
    if (recent >= OVERRIDE_LIMIT_PER_DAY)
        return deny(`rate limit reached (${OVERRIDE_LIMIT_PER_DAY}/24h) — escalate to the crew in the Observation Lounge`);
    const value = process.env[name];
    const available = Boolean(value);
...
export function summarizeOverridesForLounge(sinceHours = 24) {
    ...
    for (const [crew, n] of Object.entries(byCrew)) {
        if (n >= OVERRIDE_LIMIT_PER_DAY)
            anomalies.push(`${crew} at/over the ${OVERRIDE_LIMIT_PER_DAY}/24h override limit (${n})`);
    }
