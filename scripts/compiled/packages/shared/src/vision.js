 ...
 */
import { imageInputToUrl } from './image-input.js';
...
export function selectVisionModel(complexity, opts) {
    if (opts?.anthropic)
        return VISION_MODEL_ARCHITECTURE;
    return VISION_MODELS[complexity] ?? VISION_MODELS.moderate;
}
export const INTENT_COMPLEXITY = {
    describe: 'simple',
    extract_text: 'simple',
 ...
 */
export async function runVisionAnalysis(image, opts) {
    ...
    if (!key)
        throw new Error('no CREW_LLM_APPROVED_KEY — vision call cannot authenticate');
    ...
    for (const model of candidates) {
        try {
            ...
            if (!res.ok) {
                lastErr = new Error(`vision HTTP ${res.status}`);
                continue;
            }
            const j = await res.json(); // eslint-disable-line @typescript-eslint/no-explicit-any
