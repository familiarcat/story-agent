...
import { CREW_MISSION_ORDER, CREW_PERSONAS, type CrewId } from '../packages/mcp-server/src/lib/crew-personas.js';
import { executePromptEngineCall } from '../packages/mcp-server/src/lib/prompt-engine.js';
import { getCrewPersonalMemories, storeObservationMemory, embeddingSource } from '../packages/shared/src/db.js';

/**
 * Crew Stand-Up — a DATA-BACKED daily roll call.
 *
 *   npx tsx scripts/crew-standup.ts [--members picard,worf,...] [--memories 6] [--no-store]
 *
 * Unlike the Observation Lounge (self-referential reflection on project purpose), a stand-up asks
 * each officer the operational question a real stand-up asks: "since last time — what did you ship,
 * what's in progress, what's blocking you, what's next?" Each officer's answer is GROUNDED in their
 * actual recent RAG memories (getCrewPersonalMemories), then narrated in their canonical voice on
 * their Quark-selected OpenRouter model. Frugal by design (11 short cost-optimized calls).
 *
 ...
 * mode that leaves officers silent in the strict Observation Lounge parser.
 */
...
interface StandupEntry {
  ...
  role: string;
  model: string;
  /** Number of recent memories that grounded this report */
  memoryCount: number;
  workedOn: string;
  inProgress: string;
  blockers: string;
  next: string;
  /** True when the model did not follow the section format and we used the raw prose */
  rawFallback: boolean;
  /** True when the first call came back empty/off-format and we retried once */
  retried: boolean;
  rawText: string;
}

const STANDUP_USER_PROMPT_TEMPLATE = (activityLog: string) => `This is the crew's daily stand-up. Keep it tight and operational — this is not a philosophical reflection.
...
function standupSystemPrompt(crewId: CrewId): string {
...
function formatActivityLog(memories: Awaited<ReturnType<typeof getCrewPersonalMemories>>): string {
  ...
  return memories
    .map((m: any, i: number) => {
      ...
      // Trim content to a single grounded line so the model sees substance without token bloat.
...
async function callAndParse(crewId: CrewId, activityLog: string) {
...
async function runStandupForMember(crewId: CrewId, memoryLimit: number): Promise<StandupEntry> {
  const persona = CREW_PERSONAS[crewId];
  ...
  return {
    ...
    model: 'openrouter', // actual slug is logged by prompt-engine; not returned in result
...
function formatStandupMarkdown(entries: StandupEntry[], stardate: string): string {
  const lines: string[] = [
...
(async () => {
  ...
  const roster: CrewId[] = membersArg
    ? (membersArg.split(',').map(s => s.trim()).filter(Boolean) as CrewId[])
    : CREW_MISSION_ORDER;
